...
with open('politicsie-followedby.mtx') as f:
    for line in f:
        if i==0:
            i=i+1
            continue
...
for i in range(a):
    ...
    for j in range(0,348):
        ...
        if L[j]==-1:
            ranking[index[j]-1]=float(count/end)
        elif index[j]-1==i :
            ranking[index[j]-1]=float(end/end)
        else:
            kk=j+1
            ranking[index[j]-1]=float(kk/end)
    
    rank.append(ranking)
f=open('rankfollowedby.py','w')
sys.stdout=f
print(rank)
f.close()
